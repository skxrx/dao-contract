...
describe('DAO', function () {
  ...
  const period: number = 259200
  const minimumVotes: number = 666
  ...
  beforeEach(async function () {
    ...
    Token = await new DaoToken__factory(signers[0]).deploy()
    ...
    TContract = await new Test__factory(signers[0]).deploy()
  ...
  describe('deposit', () => {
    ...
    it('incremented activeUsers', async () => {
  ...
  describe('withdraw', () => {
    ...
    it('to emit TokensWithdrawn', async () => {
    ...
    it("reverted 'Insufficient funds'", async () => {
      await DAO.deposit(defaultAmount)
      await expect(DAO.withdrawTokens(defaultAmount + 1)).to.be.reverted
    })

    it("reverted 'last voting isn't ended'", async () => {
      await DAO.deposit(defaultAmount)
    ...
    it('if balance = 0 => decrement', async () => {
      await DAO.deposit(defaultAmount)
      const usersBefore = await DAO.getActiveUsers()
      await DAO.withdrawTokens(defaultAmount)
      const usersAfter = await DAO.getActiveUsers()
  ...
  describe('addProposal', () => {
    ...
    it('increment proposalsCounter', async () => {
    ...
    it('to emit ProposalAdded', async () => {
      await expect(
  ...
  describe('vote', () => {
    ...
    it("reverted 'already voted' ", async () => {
      await DAO.vote(0, true)
      await expect(DAO.vote(0, true)).to.be.revertedWith(
    ...
    it('usersVoted incremented', async () => {
  ...
  describe('finishProposal', () => {
    ...
    it("reverted 'Voting time is not over yet' ", async () => {
      await expect(DAO.finishProposal(0)).to.be.revertedWith(
        'DAO: Voting time is not over yet'
      )
    })

    it("reverted 'Voting has already ended' ", async () => {
      await ethers.provider.send('evm_increaseTime', [period])
    ...
    it('minimum votes less then needed ', async () => {
      await DAO.vote(0, true)
      await ethers.provider.send('evm_increaseTime', [period])
    ...
    it('changed proposal status', async () => {
      await DAO.vote(0, true)
      await ethers.provider.send('evm_increaseTime', [period])
      await DAO.finishProposal(0)
    ...
    describe('endProposal', () => {
      ...
      it("reverted 'not a contract'", async () => {
        await ethers.provider.send('evm_increaseTime', [period])
      ...
      it("to emit 'FinishedEmergency'", async () => {
        await DAO.addProposal(DAO.address, txData, 'Some description')
